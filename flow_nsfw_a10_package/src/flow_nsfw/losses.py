...
def flow_consistency_loss(
    ...
    # Warp flow_bwd by flow_fwd to check consistency
    y, x = torch.meshgrid(
    ...
    # Normalize flow
    flow_norm = flow_fwd.permute(0,1,3,4,2) / torch.tensor([W/2, H/2], device=flow_fwd.device)
    warped_grid = grid + flow_norm

    # Warp backward flow
    flow_bwd_warped = F.grid_sample(
        flow_bwd.reshape(B*Tm1, 2, H, W),
        warped_grid.reshape(B*Tm1, H, W, 2),
        align_corners=False,
        padding_mode="border",
    ).reshape(B, Tm1, 2, H, W)

    # Consistency check
    consistency_error = (flow_fwd + flow_bwd_warped).abs().mean()
...
def flow_consistency_loss(
    ...
    # Warp flow_bwd
    flow_bwd_perm = flow_bwd.float().permute(0, 1, 3, 4, 2)  # (B, Tm1, H, W, 2)
    flow_bwd_flat = flow_bwd_perm.reshape(B * Tm1, H, W, 2)
